...
namespace RubiksCubeLib.CubeModel
{
    ...
    public class Face3D
    {
        ...
        /// </summary>
        public IEnumerable<Point3D> Vertices { get; }
        ...
        /// </summary>
        public CubeFlag MasterPosition { get; }
        ...
        {
            var parr = this.Vertices.Select(v => v.Project(viewWidth, viewHeight, fov, viewDistance, scale));
            var point3Ds = parr as Point3D[] ?? parr.ToArray();
            var mid = point3Ds.Average(v => v.Z);
            parr = point3Ds.Select(p => new Point3D(p.X, p.Y, mid));
            return new Face3D(parr, this.Color, this.Position, this.MasterPosition);
        }

        public Point3D GetCenter() => new Point3D(this.Vertices.Average(v => v.X), this.Vertices.Average(v => v.Y), this.Vertices.Average(v => v.Z));
    }
}